...
namespace PomodoroAssistant.ViewModels
{
    public class TimerViewModel : ViewModelBase
    {
        ...
        // Aktualna liczba pomodoro po jakiej nastąpi długa przerwa
        private int _setLongBreakAfter;
        public int SetLongBreakAfter
        {
            get { return _setLongBreakAfter; }
            set { Set<int>(() => SetLongBreakAfter, ref _setLongBreakAfter, value); }
        }


        // Aktualny dzienny cel
        private int _setDailyTarget;
        public int SetDailyTarget
        {
            get { return _setDailyTarget; }
            set { Set<int>(() => SetDailyTarget, ref _setDailyTarget, value); }
        }
        ...
        {
            // Odczytaj ustawienia
            SetDailyTarget = _settingsInstance.GetSettings().DailyTarget;
            SetLongBreakAfter = _settingsInstance.GetSettings().PomodoroToLongBreak;
            // Ustaw czas
        ...
        private void Timer_Tick(object sender, object e)
        {
            ...
            SuspendingManager.SetSuspendingValues((int)CurrentTime.TotalSeconds, LongBreakAfter, _settingsInstance.GetSettings().PomodoroToLongBreak);
            ...
            if (CurrentTime <= TIME_END)
            {
                ...
                if (PomodoroState.GetCurrentState() == PomodoroStates.WorkTime)
                {
                    ...
                    // Zmień na przerwę
                    if (_settingsInstance.GetSettings().PomodoroToLongBreak == LongBreakAfter)
                    {
                        // Ustaw długą przerwę
                    ...
                    if (_isDailyTargetAchieved == false && _settingsInstance.GetSettings().DailyTarget == DailyTarget)
                    {
        ...
        private void SetTime(PomodoroStates currentState)
        {
            ...
            if (currentState == PomodoroStates.WorkTime)
                TotalTime = new TimeSpan(0, _settingsInstance.GetSettings().WorkDuration, 0);
            if (currentState == PomodoroStates.ShortBreakTime)
                TotalTime = new TimeSpan(0, _settingsInstance.GetSettings().ShorBreakDuration, 0);
            if (currentState == PomodoroStates.LongBreakTime)
                TotalTime = new TimeSpan(0, _settingsInstance.GetSettings().LongBreakDuration, 0);
#if DEBUG
            if (currentState == PomodoroStates.WorkTime)
                TotalTime = new TimeSpan(0, 0, _settingsInstance.GetSettings().WorkDuration);
            if (currentState == PomodoroStates.ShortBreakTime)
                TotalTime = new TimeSpan(0, 0, _settingsInstance.GetSettings().ShorBreakDuration);
            if (currentState == PomodoroStates.LongBreakTime)
                TotalTime = new TimeSpan(0, 0, _settingsInstance.GetSettings().LongBreakDuration);
#endif
